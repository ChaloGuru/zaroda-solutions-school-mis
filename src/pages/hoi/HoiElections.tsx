...
export default function HoiElections() {
  ...
  const electionsTotal = Math.ceil(elections.length / PAGE_SIZE);
  ...
  const candidatesTotal = Math.ceil(electionCandidates.length / PAGE_SIZE);
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <Select value={selectedResultElection} onValueChange={v => { setSelectedResultElection(v); setSelectedPosition(''); setVoteEdits({}); }}>
                    <SelectTrigger className="w-[250px]"><SelectValue placeholder="Select election" /></SelectTrigger>
                    <SelectContent>
                      {elections.map(e => <SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>)}
                    </SelectContent>
                  ...
                  {resultPositions.length > 0 && (
                    <Select value={selectedPosition} onValueChange={setSelectedPosition}>
                      ...
                      <SelectContent>
                        {resultPositions.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  )}
                </div>
            ...
            <CardContent>
              {!selectedResultElection ? (
                <p className="text-center text-muted-foreground py-8">Select an election to view results</p>
              ) : !selectedPosition ? (
                ...
                <div className="space-y-6">
                  ...
                  {chartData.length > 0 && (
                    <div className="mt-4">
                      <h3 className="text-lg font-semibold mb-3">Votes Chart — {selectedPosition}</h3>
