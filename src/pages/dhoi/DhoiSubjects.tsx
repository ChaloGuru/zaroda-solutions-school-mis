...
export default function DhoiSubjects() {
  ...
  const [viewSubject, setViewSubject] = useState<HoiSubject | null>(null);
  ...
  const openView = (subject: HoiSubject) => {
  ...
  const viewAssignments = viewSubject ? assignments.filter((a) => a.subject_id === viewSubject.id) : [];
  const viewTotalPages = Math.max(1, Math.ceil(viewAssignments.length / PAGE_SIZE));
  const viewPaginated = viewAssignments.slice((viewPage - 1) * PAGE_SIZE, viewPage * PAGE_SIZE);
                    ...
                    <TableRow key={subject.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="icon" onClick={() => openView(subject)}>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              {viewPaginated.length === 0 ? (
                ...
                viewPaginated.map((a) => (
                  <TableRow key={a.id}>
                    ...
                    <TableCell>
                      <code className="px-2 py-0.5 rounded bg-muted text-sm">{getTeacherCode(a.teacher_id)}</code>
                    </TableCell>
                  </TableRow>
                ))
              )}
          ...
          {viewTotalPages > 1 && (
            <div className="flex items-center justify-between mt-2">
              <p className="text-sm text-muted-foreground">Page {viewPage} of {viewTotalPages}</p>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" disabled={viewPage === 1} onClick={() => setViewPage(viewPage - 1)}>
                  <ChevronLeft className="w-4 h-4" />
                </Button>
                <Button variant="outline" size="sm" disabled={viewPage === viewTotalPages} onClick={() => setViewPage(viewPage + 1)}>
                  <ChevronRight className="w-4 h-4" />
                </Button>
              </div>
            </div>
